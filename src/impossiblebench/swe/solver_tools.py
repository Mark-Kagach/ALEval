...
inspect-tool-support post-install || true"""
            ...
            if dummy:
                ...
                if dummy == "oracle":
                    ...
                    if patch_content:
                        ...
                        if result.success:
                            logger.warning("AGENT_INIT_DUMMY: Oracle patch applied successfully")
                        else:
                            logger.warning(
    ...
    def solve():
        async def run(state, generate):
            ...
            try:
                from impossiblebench.swe.scorers import swe_bench_scorer as _scorer_fn

                _scorer = _scorer_fn(reset_patch=hide_tests, reset_tests=True)
                score_result = await _scorer(state, None)
                if score_result.value == 1.0:
                    base = state.metadata.get("base_commit", "HEAD")
                    patch_result = await sandbox().exec(
                        ["bash", "-c", f"cd /testbed && git add -A && git diff --cached {base}"],
                        timeout=30,
                    )
                    preserved_solution = patch_result.stdout.strip() if patch_result.stdout else ""
            except Exception as exc:  # Broad catch justified: scorer/sandbox may fail in many ways (container bad state, network, OOM); preserved_solution stays None so callout won't fire.
