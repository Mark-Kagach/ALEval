...
def get_dump_of_everything(s):
    ...
    for msg in s.messages:
        ...
        for tool_call in tool_calls:
            dump_of_everything += (
                f"[TOOL CALL: {tool_call.function}, ARGS: {tool_call.arguments}]\n"
            )
        dump_of_everything += "\n\n"
...
class EvalResult:
    ...
    timestamp: str | None
    sample_id: str | None
    sample_original_id: str | None
    task_name: str | None
    dump: str | None
    callout_prompt: str | None = None
    callout_response: str | None = None
    callout_deterministic_label: str | None = None
...
def parse_task_display_name(task_display_name: str) -> dict[str, str | None]:
    ...
    if task_display_name.startswith("aleval_"):
        ...
        # ALEval presets always use conflicting split with callout; set early default
        _aleval_preset = True
    else:
        _aleval_preset = False
    ...
    for _i, part in enumerate(parts[1:], 1):  # Skip dataset (first part)
        ...
        if samples:
            for i, sample in enumerate(samples):
                ...
                if hasattr(sample, "scores") and sample.scores:
                    ...
                    for _scorer_name, score_result in sample.scores.items():

                        def score_to_float(v):
                            return float({"I": 0, "C": 1}.get(v, v))
                        ...
                        if isinstance(score_metadata, dict):
                            ...
                            sample_callout_deterministic_label = score_metadata.get(
...
class DataLoader:
    ...
    def to_df(self, dump=False) -> pd.DataFrame:
        ...
        if "callout_deterministic_label" in df.columns:
            df["callout_deterministic_label"] = df["callout_deterministic_label"].fillna(
                "not_applicable"
            )

        # Add computed columns only for aggregated rows (sample_id=None)
        df["pass_rate"] = df.groupby(
            ["model", "dataset", "variant", "agent_type", "prompt_id", "modification"]
        )["passed"].transform(lambda x: x.mean(skipna=True))
        df["first_pass_rate"] = df.groupby(
            ["model", "dataset", "variant", "agent_type", "prompt_id", "modification"]
        )["first_pass"].transform(lambda x: x.mean(skipna=True))
        df["count"] = df.groupby(
            ["model", "dataset", "variant", "agent_type", "prompt_id", "modification"]
        )["passed"].transform(lambda x: x.count())
        ...
        pass_rate_err = df.groupby(
            ["model", "dataset", "variant", "agent_type", "prompt_id", "modification"]
        )["passed"].transform(calculate_standard_error)
        first_pass_rate_err = df.groupby(
            ["model", "dataset", "variant", "agent_type", "prompt_id", "modification"]
        )["first_pass"].transform(calculate_standard_error)
