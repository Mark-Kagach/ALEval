...
DEFAULT_LOG_ROOT="./logs/aleval_lcb_min_legacy_models"
LEGACY_LOG_ROOT="./logs/aleval_lcb_min_gpt41mini_peers"
DEFAULT_REPORT_ROOT="./reports/aleval_lcb_min_legacy_models"
LEGACY_REPORT_ROOT="./reports/aleval_lcb_min_gpt41mini_peers"
...
print_config() {
  sync_compat_aliases
  echo "LCB-min legacy models configuration"
  echo "  task:              $TASK"
...
status() {
  sync_compat_aliases
  echo "LCB-min legacy models panel status"
  for model in "${MODELS[@]}"; do
    local model_safe status_value eval_count logs_dir missing_count sample_ids
    model_safe="$(sanitize_model "$model")"
    ...
    printf '  %-45s  status=%-12s eval_files=%s missing=%s\n' "$model" "$status_value" "$eval_count" "$missing_count"
  done
}
...
usage() {
  cat <<USAGE
Usage: ./experiments/run_lcb_min_legacy_models.sh [run|resume|report|all|status|print-config]

Commands:
  run           Run the legacy models panel in parallel.
  resume        Resume only samples missing from existing eval logs.
  report        Generate the canonical per-model + combined report bundle.
  all           Run the legacy models panel, then generate the report bundle.
...
Environment overrides:
  ...
  LOG_ROOT             Default: ./logs/aleval_lcb_min_legacy_models
  REPORT_ROOT          Default: ./reports/aleval_lcb_min_legacy_models
  MAX_ATTEMPTS         Default: 5
  MESSAGE_LIMIT        Default: 60
  MAX_CONNECTIONS      Default: 16
  MAX_PARALLEL_MODELS  Default: 4
